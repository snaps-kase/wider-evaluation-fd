...
def eager_evaluation(pred_dir, gt_path, iou_thresh=0.5):
    ...
    for event in pbar:
        ...
        event_images = os.listdir(event_dir)
        current_event = dict()
        url = "http://stg-ai.snaps.com:10030/v1/smart/lab/fd"
        for imgname in event_images:
            jpgtxt = base64.b64encode(open(f"{event_dir}/{imgname}", "rb").read()).decode('utf-8')
            data = {
                "imageData": jpgtxt,
                "types": "bytes",
                "ot": 1
            ...
            current_event[imgname.rstrip('.jpg')] = np.array([[r['x1'], r['y1'], r['x2']-r['x1'], r['y2']-r['y1'],
                                                                            r['confidence']]for r in result['faces']])
        pred[event] = current_event
    norm_score(pred)
    ...
    for i in pbar:
        ...
        aps.append(ap)
    print("==================== Results ====================")
    print("Medium  Val mAP: {}".format(aps[0]))
    print("=================================================")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--pred', default='./wider_val')
    parser.add_argument('-g', '--gt', default='./ground_truth/')

    args = parser.parse_args()
    eager_evaluation(args.pred, args.gt)

    # wider_val_images_path = './wider_val/'
